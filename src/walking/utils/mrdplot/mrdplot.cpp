...
char generated_file_name[1000];

char * generate_file_name(const char *pathname, const char *filename)
{
	FILE *stream;
	char tmp_string_time[100];
	char tmp_string_user[100];
	int return_nowarn;
	int file_number = 0;
	time_t now = time(0);
	struct tm tstruct;

	const char * lastdata_filename = "last_data";
	char * lastdata_path = (char *) malloc(1 + strlen(pathname) + strlen(lastdata_filename) );
	strcpy(lastdata_path, pathname);
	strcat(lastdata_path, lastdata_filename);

	stream = fopen( lastdata_path, "r" );
	if ( stream != NULL )
	{
	...
	const char * mrdplot_data_postfix = "mrdplot/%s_%s_%s.mrd";
	char * mrdplot_data_path = (char *) malloc(1 + strlen(pathname) + strlen(mrdplot_data_postfix) );
	strcpy(mrdplot_data_path, pathname);
	strcat(mrdplot_data_path, mrdplot_data_postfix);

	sprintf(generated_file_name, mrdplot_data_path, filename, tmp_string_user, tmp_string_time);

	stream = fopen( lastdata_path, "w" );
	...
	const char * sh_command_prefix = "chmod 666 ";
	char * sh_command = (char *) malloc(1 + strlen(sh_command_prefix) + strlen(lastdata_path) );
	strcpy(sh_command, sh_command_prefix);
	strcat(sh_command, lastdata_path);

	return_nowarn = system(sh_command);
	if ( stream != NULL )
	{
		file_number++;
		fprintf( stream, "%d\n", file_number );
